...
function match(object, matcherOrValue) {
    ...
    if (isMap(object)) {
        return isMap(matcherOrValue) && matchMap(object, matcherOrValue);
    }
...
function matchMap(map1, map2) {
    var firstMap = new ES6Map(map1);
    var secondMap = new ES6Map(map2);
    var firstKeys = [];

    // filter the keys of map1 to only be the keys that also exist in map2
    arrayFrom(firstMap.keys()).forEach(function(key) {
        if (secondMap.has(key)) {
            firstKeys.push(key);
        }
    });

    if (firstKeys.length !== arrayFrom(secondMap.keys()).length) {
        return false;
    }

    var inOrder = true;
    var i = 0;
    arrayFrom(secondMap.keys()).forEach(function(key) {
        inOrder = inOrder && firstKeys[i] === key;
        i++;
    });
    ...
    var results = [];
    secondMap.forEach(function(value, key) {
        if (!firstMap.has(key)) {
            results.push(false);
        }
        var valueFrom1 = firstMap.get(key);

        results.push(match(valueFrom1, value));
    });
